...
class AlexNet(object):
    ...
    def construct(self):
        ...
        # 卷积 输入shape 224*224*3 输出shape 55*55*96
        conv_1 = self.conv(X, 'conv_1', 11, 11, 4, 4, 96)

        # 最大值池化 输出shape 27*27*96
        pool_2 = self.max_pool(conv_1, 'pool_2', 3, 3, 2, 2)

        # 卷积 输出shape 27*27*256
        conv_3 = self.conv(pool_2, 'conv_3', 5, 5, 1, 1, 256)

        # 最大值池化 输出shape 13*13*256
        pool_4 = self.max_pool(conv_3, 'pool_4', 3, 3, 2, 2)

        # 卷积 输出shape 13*13*384
        conv_5 = self.conv(pool_4, 'conv_5', 3, 3, 1, 1, 384)
        conv_6 = self.conv(conv_5, 'conv_6', 3, 3, 1, 1, 384)

        # 卷积 输出shape 13*13*256
        conv_7 = self.conv(conv_6, 'conv_7', 3, 3, 1, 1, 256)

        # 最大值池化 输出shape 6*6*256
        pool_8 = self.max_pool(conv_7, 'pool_8', 3, 3, 2, 2)
        ...
        # 全连接层
        fc_9 = self.fc(reshaped, 'fc_9', 4096)
        relu_fc_9 = tf.nn.relu(fc_9)

        fc_10 = self.fc(relu_fc_9, 'fc_10', 4096)
        relu_fc_10 = tf.nn.relu(fc_10)

        fc_output = self.fc(relu_fc_10, 'fc_output', 10)
        self.p = tf.nn.softmax(fc_output)
...
def main():
    mlr = AlexNet()
    mlr.fit(epoch=20, batch_size=50)
    mlr.save('AlexNet_model')
    mlr.plot('AlexNet')
